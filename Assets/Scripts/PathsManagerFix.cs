...
public class PathsManagerFix : MonoBehaviour
{
    ...
    // Auxiliares
    private GameObject pathSelected;
    ...
    private void Start()
    {
        AddKnotTo(pathA.transform.GetChild(0), new Vector3(0f, 0.02f, 0.02f));
        AddKnotTo(pathB.transform.GetChild(0), new Vector3(0f, 0.02f, 0.02f));
    }
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && isUnionMode)
        {
            ...
            if (Physics.Raycast(ray, out raycastHitInfo))
            {
                ...
                string targetName = stateTarget.parent.GetComponentInChildren<PathsManagerFix>().stateName.text;
                UserData.instance.SOLUTION_MOD2[stateName.text + "," + pathSelected.name] = targetName;
            }
    ...
    public void CreatePhat()
    {
        ...
        Transform transitionPoints = pathSelected.name.Equals(Constants.instance.SYMBOL_A_NAME) ? auxPointsA : auxPointsB;
